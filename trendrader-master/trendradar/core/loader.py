...
def _load_topics_config(config_data: Dict) -> Dict:
    """加载主题追踪配置"""
    topics_config = config_data.get("topics", {})

    return {
        "ENABLED": topics_config.get("enabled", False),
        "CONFIG_FILE": topics_config.get("config_file", "config/topics.yaml"),
        "ARCHIVE_DAYS": topics_config.get("archive_days", 7),
        "MAX_ITEMS_PER_TOPIC": topics_config.get("max_items_per_topic", 10),
...
def load_config(config_path: Optional[str] = None) -> Dict[str, Any]:
    ...
    # 主题追踪配置
    config["TOPICS"] = _load_topics_config(config_data)
