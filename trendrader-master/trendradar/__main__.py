...
class NewsAnalyzer:
    ...
    def _run_ai_analysis(
        ...
        try:
            ...
            result = analyzer.analyze(
                ...
                platforms=platforms,
                keywords=keywords,
                history_context=history_context,  # 传入历史上下文
            )
    ...
    def _run_topic_tracking(
        ...
        Returns:
            (topics_summary, history_context) 元组
            - topics_summary: 主题摘要数据，用于推送
            - history_context: 历史上下文文本，用于AI分析
        """
        topics_config = self.ctx.config.get("TOPICS", {})
        if not topics_config.get("ENABLED", False):
            return None, None

        try:
            from trendradar.topic import TopicClassifier, TopicStorage, HistorySummarizer
            from trendradar.utils.time import get_recent_dates

            print("[主题] 正在进行主题分类...")

            # 初始化组件
            config_file = topics_config.get("CONFIG_FILE", "config/topics.yaml")
            base_dir = topics_config.get("BASE_DIR", "output/topics")
            archive_days = topics_config.get("ARCHIVE_DAYS", 7)

            classifier = TopicClassifier(config_file)
            storage = TopicStorage(base_dir, self.ctx.timezone)
            summarizer = HistorySummarizer(storage, self.ctx.timezone)

            # 对热榜数据进行主题分类
            topic_results = classifier.classify(data_source, rss_items)
            ...
            # 保存到 Markdown 文件
            current_date = self.ctx.format_date()
            topics_summary = {}

            for topic_id, items in topic_results.items():
                ...
                # 限制条目数量
                max_items = topics_config.get("MAX_ITEMS_PER_TOPIC", 20)
                if max_items > 0 and len(items) > max_items:
                    items = items[:max_items]

                # 保存到文件
                file_path = storage.save_topic_classification(
                    date=current_date,
                    topic_id=topic_id,
                ...
                print(f"[主题] {topic_name}: {len(items)} 条 → {file_path}")

            # 生成历史摘要（用于AI分析）
            history_context = None
            if archive_days > 0:
                print(f"[主题] 正在生成最近 {archive_days} 天的历史摘要...")
                history_context = summarizer.get_recent_summaries(
                    days=archive_days,
                    current_date=current_date
                )
                if history_context:
                    print(f"[主题] 历史摘要已生成（{len(history_context)} 字符）")
                else:
                    print("[主题] 暂无历史数据")

            return topics_summary, history_context
    ...
    def _run_analysis_pipeline(
        ...
        """统一的分析流水线：数据处理 → 统计计算 → 主题追踪 → AI分析 → HTML生成"""
        ...
        if topics_config.get("ENABLED", False):
            topics_summary, history_context = self._run_topic_tracking(
                data_source, rss_items
            )
